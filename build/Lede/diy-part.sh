...
export DNS_Settings="10.0.0.10"                     # 旁路由设置 DNS(填入DNS，多个DNS要用空格分开)(填0为不作修改)
...
rm -rf feeds/danshui1/cdnspeedtest
rm -rf package/feeds/danshui1/cdnspeedtest
...
sed -i 's/"nezha agent"/"哪吒监控"/g' `egrep "nezha agent" -rl ./`
sed -i 's/"Control"/"网络控制"/g' `egrep "Control" -rl ./`
